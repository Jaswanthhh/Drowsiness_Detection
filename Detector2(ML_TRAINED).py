# Save this as drowsiness_gui.py and run it
...
def detect():
    ...
    while shared_state["running"]:
        ...
        if result.multi_face_landmarks:
            ...
            l_eye = [(face[i].x*w, face[i].y*h) for i in LEFT_EYE_IDX_MEDIAPIPE]
            r_eye = [(face[i].x*w, face[i].y*h) for i in RIGHT_EYE_IDX_MEDIAPIPE]
            ear = (eye_aspect_ratio(l_eye) + eye_aspect_ratio(r_eye)) / 2.0
            eye_open = ear >= CONFIG["thresh"]

            image_points = np.array([(face[i].x*w, face[i].y*h) for i in HEAD_IDX_MEDIAPIPE], dtype="double")
            cam_matrix = np.array([[w, 0, w/2], [0, w, h/2], [0, 0, 1]], dtype="double")
            _, rvec, tvec = cv2.solvePnP(MODEL_MEDIAPIPE, image_points, cam_matrix, np.zeros((4, 1)))
